# AstrBot 俄罗斯轮盘游戏插件 (`astrbot_plugin_russian_roulette`)

[![AstrBot Plugin](https://img.shields.io/badge/AstrBot-Plugin-blue.svg)](https://github.com/Soulter/AstrBot)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)

面向 **AstrBot** 聊天机器人生态的**俄罗斯轮盘 (Russian Roulette)** 策略对战主持人与战况展示插件。

群聊成员可以通过 `/rr` 或 `/轮盘` 指令直接在 QQ / 微信等聊天窗口中发起对决、移动走位、瞄准扣动扳机并体验紧张刺激的战术对战！
...
- **灵活双模架构**：
  - **公网/远程连接**：支持直接连接已部署的公网 MCP 轮盘游戏服务器，适合云端多群联机与集中主持；
  - **本地模式**：可连接本地运行的轮盘服务端（`http://127.0.0.1:8787`），后续版本亦将支持在插件内部自主拉起本地轻量 MCP 游戏引擎；
...
2. 进入「插件」页面，在搜索框中输入 `俄罗斯轮盘` 或 `astrbot_plugin_russian_roulette`；
3. 点击「安装」即可。
...
| 配置项 | 类型 | 默认值 | 说明 |
| :--- | :--- | :--- | :--- |
| `server_url` | string | `http://127.0.0.1:8787` | 俄罗斯轮盘游戏后端地址 (支持公网服务器或本地开发服务) |
